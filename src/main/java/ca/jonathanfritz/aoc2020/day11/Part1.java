...
public class Part1 {
            ...
            for (int row = 0; row < seats.length; row++) {
                for (int col = 0; col < seats[row].length; col++) {
                    switch (seats[row][col]) {
                        case AVAILABLE:
                            if (getNumAdjacentOccupiedSeats(seats, row, col) == 0) {
                                temp[row][col] = TAKEN;
                                changed = true;
                            }
                            break;
                        case TAKEN:
                            if (getNumAdjacentOccupiedSeats(seats, row, col) >= 4) {
    ...
    private int getNumAdjacentOccupiedSeats(String[][] seats, int row, int col) {
        return isOccupied(seats, row - 1, col)          // north
            + isOccupied(seats, row - 1, col + 1)   // north east
            + isOccupied(seats, row, col + 1)           // east
            + isOccupied(seats, row + 1, col + 1)   // south east
            + isOccupied(seats, row + 1, col)           // south
            + isOccupied(seats, row + 1, col - 1)   // south west
            + isOccupied(seats, row, col -1)            // west
            + isOccupied(seats, row - 1, col - 1);  // north west

    }

    private int isOccupied(String[][] seats, int row, int col) {
        if (row < 0 || col < 0 || row == seats.length || col == seats[row].length) {
            // if seat is out of bounds, it is not considered to be occupied
            return 0;
        } else {
            return TAKEN.equalsIgnoreCase(seats[row][col]) ? 1 : 0;
        }
    }
